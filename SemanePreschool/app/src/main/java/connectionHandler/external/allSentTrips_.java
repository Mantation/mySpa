...
public class allSentTrips_ extends Application {
    ...
    public static AdminTripAdapter adminTripAdapter;

    public static void getAllTripsDocuments(final Activity activity, final RecyclerView recyclerView,final TextView textView) {
        clearList(topic);
        ...
        getTrips(activity,recyclerView, textView);
        //getCommentIssues(activity, context, view, recyclerView); - should be like this
    }



    public static void checkTrip(final Activity activity, final RecyclerView recyclerView, final TextView textView){
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            for (DocumentSnapshot document : task.getResult()) {
                                ...
                                document.get("submission date") != null && document.get("submission time") != null && document.get("price") != null) {
                            ...
                            String[] myprice = new String[price.size()];
                            ...
                            for (int i = 0; i < mytopic.length; i++) {
                                for (int j = i; j < mytopic.length - 1; j++) {
                                    ...
                                    if(MyOldDate<MyDate)
                                    {
                                        ...
                                        tempprice = myprice [j + 1];
                                        myprice [j + 1]= myprice [i];
                                        myprice [i] = tempprice;
                            ...
                            adminTripAdapter = new AdminTripAdapter(activity, activity);
                            adminTripAdapter.setTopic(mytopic);
                            adminTripAdapter.setAttendants(myattendance);
                            adminTripAdapter.setVenue(myvenue);
                            adminTripAdapter.setPrice(myprice);
                            adminTripAdapter.setDate(mydate);
                            adminTripAdapter.setTime(mytime);
                            adminTripAdapter.setSubmissionDate(mySubdate);
                            adminTripAdapter.setSubmissionTime(mySubtime);
                            recyclerView.setLayoutManager(new LinearLayoutManager(activity));
                            recyclerView.setAdapter(adminTripAdapter);
                            if(mytopic.length == 0)
    ...
    private static  void getTrips(final Activity activity, final RecyclerView recyclerView, final TextView textView){
                ...
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            ...
                            checkTrip(activity,recyclerView,textView);
                        }
                    }
                });
    }
